...
Use this checklist from a clean `main` worktree for the local `1.5.5` release
candidate.
...
4. Run `omazen doctor` and `omazen doctor --json`; require zero failures and
   ...
   `omazen doctor --json > /tmp/omazen-1.5.5-doctor.json`.
5. Confirm `bridge.log` contains `BRIDGE_LOADED version=1.5.5`,
